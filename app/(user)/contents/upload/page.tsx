...
export default function UploadPage() {
  ...
  const handleUploadAll = async () => {
    ...
    // Redirect to contents page after 1 second
    setTimeout(() => {
      ...
      <Card className="p-8">
        ...
        >
          ...
          {isDragActive ? (
            ...
            <>
              <p className="mb-2 text-lg font-medium">파일을 드래그하거나 클릭하여 선택하세요</p>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setUploadItems([])} disabled={isUploading}>
                전체 삭제
              </Button>
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    {item.status === 'uploading' && (
                      <div className="space-y-2">
                        ...
                        <p className="text-secondary-500 text-sm">업로드 중... {item.progress}%</p>
                      </div>
                    )}

                    {/* Error */}
                    {item.status === 'error' && <p className="text-error text-sm">{item.error}</p>}
